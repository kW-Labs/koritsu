...
class SCEAddThermalMassPreheatCoolTest < Minitest::Test
  # def setup
  # end
  ...
  def run_measure_test(test_name, osm_path, epw_path, args, max_unmet_hrs: 300)
    ...
    if result.value.valueName == 'Success'
      args['template'].nil? ? template = '90.1-2013' : template = args['template']
  ...
  def apply_measure_to_model(runner, test_name, args, model_name = nil, result_value = 'Success', warnings_count = nil, info_count = nil)
  ...
  def skip_test_good_argument_values
    # create an instance of the measure
    measure = SCEAddThermalMassPreheatCool.new
