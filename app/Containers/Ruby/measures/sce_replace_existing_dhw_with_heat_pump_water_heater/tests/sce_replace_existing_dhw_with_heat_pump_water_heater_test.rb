...
class SCEReplaceExistingDHWWithHeatPumpWaterHeaterTest < Minitest::Test
  # def setup
  # end
  ...
  def apply_measure_to_model(runner, test_name, args, model_name = nil, result_value = 'Success', warnings_count = 0, info_count = nil)
    ...
    measure = SCEReplaceExistingDHWWithHeatPumpWaterHeater.new
  ...
  def test_number_of_arguments_and_argument_names
    ...
    count = -1
    assert_equal('vol', arguments[count += 1].name)
    assert_equal('cap', arguments[count += 1].name)
    assert_equal('cop', arguments[count += 1].name)
    assert_equal('osz_factor', arguments[count += 1].name)
    assert_equal('bu_cap', arguments[count += 1].name)
    assert_equal('flex0', arguments[count += 1].name)
    assert_equal('flex_hrs0', arguments[count += 1].name)
    assert_equal('flex1', arguments[count += 1].name)
    assert_equal('flex_hrs1', arguments[count += 1].name)
    assert_equal('flex2', arguments[count += 1].name)
    assert_equal('flex_hrs2', arguments[count += 1].name)
    assert_equal('flex3', arguments[count += 1].name)
    assert_equal('flex_hrs3', arguments[count += 1].name)
    assert_equal('max_temp', arguments[count += 1].name)
    assert_equal('min_temp', arguments[count += 1].name)
    assert_equal('db_temp', arguments[count += 1].name)
  end
  ...
  def test_good_argument_values
    ...
    args_hash = {'vol'=> 100,
                ...
                'osz_factor' => 1.2,
                'bu_cap' => 50,
                'flex0' => 'Charge - Heat Pump',
                'flex1' => 'Charge - Electric',
                'flex2' => 'Float',
                'flex3' => 'None',
                'flex_hrs0' => '00:00 - 12:00',
                'flex_hrs1' => '12:00 - 02:00',
                'flex_hrs2' => '02:00 - 04:00',
              }
  ...
  def skip_test_and_run_hpwh_defaults
