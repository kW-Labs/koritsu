...
# start the measure
class SCEReplaceExistingHVACWithWaterSourceHeatPumps < OpenStudio::Measure::ModelMeasure
  ...
  def name
    ...
    return 'SCE Replace Existing HVAC with Water-Source Heat Pumps'
  end

  # human readable description
  def description
    return 'Replaces building HVAC system with one of a selection of WSHP system configurations. System cooling and heating rated efficiency will be user inputs. Primary equipment (chiller/boiler, cooling tower, etc) efficiency will meet DEER parameters according to the user-entered DEER vintage'
  end

  # human readable description of modeling approach
  def modeler_description
    return 'The measure will remove all the existing building HVAC systems and replace them with the user-specifed system. A water-source heat pump will be added for each zone in the model. The option to add a dedicated outdoor air system (DOAS) will add one per floor to provide pre-conditioned ventilation air to the zones. DOAS cooling source can be either packaged DX or WSHP; DOAS heating source can be gas furnace, electric resistance, packaged DX (heat pump) or WSHP. User will be able to input efficiencies for DOAS cooling and heating source, to be applied to all DOASs.
...
'
  ...
  def arguments(model)
    ...
    #WSHP Loop Cooilng Source
    loop_cool = OpenStudio::Measure::OSArgument.makeChoiceArgument('loop_cool', ['Cooling Tower', 'Fluid Cooler'], true)
    loop_cool.setDisplayName("WSHP Loop Heat Rejection Equipment")
    loop_cool.setDescription("Type of equipment used to reject heat from the WSHP water loop")
    loop_cool.setDefaultValue('Cooling Tower')
    args << loop_cool

    #WSHP Loop Heating Source
    loop_heat = OpenStudio::Measure::OSArgument.makeChoiceArgument('loop_heat', ['Gas Boiler', 'Electric Boiler', 'Central Air-Water Heat Pump'], true)
    loop_heat.setDisplayName("WSHP Loop Heating Equipment")
    loop_heat.setDescription("Type of equipment used to add heat to the WSHP water loop")
    loop_heat.setDefaultValue('Gas Boiler')
    args << loop_heat

    #WSHP Loop Heating Efficiency
    loop_heat_cop = OpenStudio::Measure::OSArgument.makeDoubleArgument('loop_heat_cop', true)
    loop_heat_cop.setDisplayName("WSHP Heating Source Rated Efficiency")
    loop_heat_cop.setDescription("This value will be applied to WSHP loop heating equipment.\nFor gas boiler, enter a value less than 1 as boiler thermal efficiency.\nFor Electric boiler, enter 1.0.\nFor Central Air-water heat pump this is rated COP (>1).")
    loop_heat_cop.setDefaultValue(1)
    args << loop_heat_cop

    # WSHP Cooling COP
    cool_cop = OpenStudio::Measure::OSArgument.makeDoubleArgument('cool_cop', true)
    cool_cop.setDisplayName("Water-Source Heat Pump Cooling Rated COP")
    cool_cop.setDescription("This value will be applied to all zone-level WSHP equipment in cooling.")
    ...
    heat_cop.setDescription("This value will be applied to all zone-level WSHP equipment in heating.")
  ...
  def run(model, runner, user_arguments)
    ...
    loop_cool = args['loop_cool']
    loop_heat = args['loop_heat']
    loop_heat_cop = args['loop_heat_cop']
    ...
    # system add logic
    # first add loop
    hp_loop_heat_fuel_map = {'Gas Boiler'=>'NaturalGas', 'Electric Boiler'=>'Electricity', 'Central Air-Water Heat Pump'=>'AirSourceHeatPump'}
    hp_loop_cool_fuel_map = {'Cooling Tower'=>'CoolingTowerVariableSpeed', 'Fluid Cooler'=> 'FluidCoolerTwoSpeed', 'Evaporative Cooler'=>'EvaporativeFluidCoolerTwoSpeed'}
    hp_loop = std.model_add_hp_loop(model, heating_fuel: hp_loop_heat_fuel_map[loop_heat], cooling_type: hp_loop_cool_fuel_map[loop_cool])
    ...
    # create WSHPs
    wshps = std.model_add_water_source_hp(model, system_zones, hp_loop, ventilation: zone_vent)
    ...
    # set user input efficiencies
    # plant heating
    hp_loop.supplyComponents.each do |eqp|
      if eqp.iddObjectType.valueName.include? "Boiler"
        eqp = eqp.to_BoilerHotWater.get
        eqp.setNominalThermalEfficiency(loop_heat_cop)
      elsif eqp.iddObjectType.valueName.include? "UserDefined"
        ...
        model.getEnergyManagementSystemCurveOrTableIndexVariables.each do |var|
          ...
          if curve.to_CurveQuadratic.is_initialized
            ...
            constant_coeff_new = 1.932 + (loop_heat_cop - 3.65)
    ...
    # log_msgs_to_file(debug, "add_wshp_output")
    return true
  end
end

# register the measure to be used by the application
SCEReplaceExistingHVACWithWaterSourceHeatPumps.new.registerWithApplication
