...
class SCEReplaceExistingHVACWithAirSourceHeatPumpsTest < Minitest::Test
  # def setup
  # end
  ...
  def test_number_of_arguments_and_argument_names
    # create an instance of the measure
    measure = SCEReplaceExistingHVACWithAirSourceHeatPumps.new
    ...
    assert_equal('system_type', arguments[1].name)
    assert_equal('cool_cop', arguments[2].name)
    assert_equal('heat_cop', arguments[3].name)
  end
  ...
  def test_bad_argument_values
    ...
    args_hash = {}
    args_hash['template'] = 'DEER 2015'
    args_hash['system_type'] = 'Packaged Single-Zone Heat Pumps (i.e. split systems)'
    args_hash['cool_cop'] = -2
    args_hash['heat_cop'] = 4.0
  ...
  def test_psz_hp
    ...
    osm_path = File.dirnmae(__FILE__)+'/in.osm'
    ...
    # load the test model
    translator = OpenStudio::OSVersion::VersionTranslator.new
    path = "#{File.dirname(__FILE__)}/in.osm"
    model = translator.loadModel(path)
    assert(!model.empty?)
    model = model.get
    ...
    # save the model to test output directory
    output_file_path = "#{File.dirname(__FILE__)}//output/test_psz_hp_output.osm"
    model.save(output_file_path, true)
  end
  ...
  def test_psz_hydro
    ...
    args_hash = {}
    args_hash['template'] = 'DEER 2015'
    args_hash['system_type'] = 'Packaged Single-Zone AC with Hydronic Heating via Central Air-Water Heat Pump'
    args_hash['cool_cop'] = 3.5
    args_hash['heat_cop'] = 4.0
    # using defaults values from measure.rb for other arguments
    ...
    # save the model to test output directory
    output_file_path = "#{File.dirname(__FILE__)}//output/test_psz_hdyro_output.osm"
    model.save(output_file_path, true)
  end
  ...
  def test_doas_fcu
    ...
    args_hash = {}
    args_hash['template'] = 'DEER 2015'
    args_hash['system_type'] = 'DOAS with Fan Coil Units, CHW via Air-Cooled Chiller, Heating via Central Air-Water Heat Pump'
    args_hash['cool_cop'] = 3.5
    args_hash['heat_cop'] = 4.0
    # using defaults values from measure.rb for other arguments
    ...
    # save the model to test output directory
    output_file_path = "#{File.dirname(__FILE__)}//output/test_doas_fcu_output.osm"
    model.save(output_file_path, true)
  end
  ...
  def test_vav
    ...
    args_hash = {}
    args_hash['template'] = 'DEER 2015'
    args_hash['system_type'] = 'VAV AHU, CHW via Air-Cooled Chiller, Heating and Reheat via Central Air-Water Heat Pump'
    args_hash['cool_cop'] = 3.5
    args_hash['heat_cop'] = 4.0
    # using defaults values from measure.rb for other arguments
    ...
    # save the model to test output directory
    output_file_path = "#{File.dirname(__FILE__)}//output/test_vav_output.osm"
    model.save(output_file_path, true)
  end
