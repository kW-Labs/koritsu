...
class SCEAddElectricStorageTest < Minitest::Test
  # def setup
  # end
  ...
  def test_demand_leveling
    # create an instance of the measure
    measure = SCEAddElectricStorage.new
    test_name = (__method__).to_s
    args = {
      "stor_op" => "Facility Demand Leveling",
      "demand_target" => 100,
    }
    model, result = apply_measure_to_model(runner = nil, test_name = (__method__), args, model_name = nil, result_value = 'Success', warnings_count = 0, info_count = nil)
    puts model.getElectricLoadCenterDistributions
    puts model.getElectricLoadCenterStorageSimples
  end
